...
export DSSC_ECR_REPOID=$(curl -s -k -X POST https://${DSSC_HOST}/api/registries?scan=false -H "Content-Type: application/json" -H "Api-Version: 2018-05-01" -H "Authorization: Bearer ${DSSC_BEARERTOKEN}" -H 'cache-control: no-cache' -d  "{  \"name\": \"ECR_${AWS_PROJECT}\",  \"description\": \"Added by ${AWS_PROJECT} \n\",  \"credentials\": {     \"aws\": {       \"region\": \"${AWS_REGION}\" }  },  \"insecureSkipVerify\": true,   \"filter\": {    \"include\": [      \"*\"    ]  }, \"schedule\": true}" | jq '.id')
printf '%s\n' $DSSC_ECR_REPOID
