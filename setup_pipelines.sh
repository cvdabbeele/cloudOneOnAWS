...
function create_pipeline_yaml {
  #creates the git CodeRepo (but without branch, no data), CodePipeline and ECR registry
  #$1 = name of the pipeline (${AWS_PROJECT}${APP[$i]})
  #$2 = the number of the app  (having to simulate arrays here as they cannot be exported)
...
for i in "${!aws_pipeline_stacks[@]}"; do
  ...
  if [[ "${aws_pipeline_stacks[$i]}" =~ "${1}Pipeline" ]]; then
    ...
    if [[ "${aws_pipeline_stack_exists}" = "true" ]]; then
      ...
      printf '%s \n' "Waiting for old Cloudformation Stack to be deleted: ${aws_pipeline_stack}"
